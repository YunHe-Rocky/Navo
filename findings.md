...
## 2026-08-31 Phase 73 initial findings

- The requested scope has three coupled acceptance points: persisted log classification, user-controlled level-based handling/filtering, and a deterministic Basic Service default when the log view opens.
- Existing project history already added safe structured evidence rendering for method, error code, reason, and request ID; Phase 73 must extend that contract without rendering arbitrary fields or weakening redaction.
- The repository was clean at task start. No Navo process, proxy listener, WinINet setting, route, DNS, firewall, adapter, or unrelated user process has been changed.
- Product wording is initially interpreted conservatively: “分级处理” means user-selectable querying/filtering by severity, not destructive deletion or rewriting of persisted records. Source inspection will confirm the existing UI vocabulary before implementation.
- The existing canonical store already persists one JSONL record with explicit `level`, `service`, `component`, redacted message/fields, UTC timestamp, bounded 5 MiB rewrite, and a 2,000-entry in-memory cap. It currently supports DEBUG/INFO/WARN/ERROR and backend filtering by levels, raw service names, time, cursor, and limit.
- The Settings UI already exposes level/service checkboxes, time bounds, query, follow, clear-visible, and clear-persisted controls. Its current defaults are levels INFO/WARN/ERROR and no service selection, which means every service; changing a checkbox does not query until the user presses “查询”.
- No source or product copy currently defines “基础服务”. Persisted producer names include Launcher, Agent, Service, and SelfHeal, while Service request-received records may be classified further by `logServiceForMethod`.
- The smallest compatible interpretation is to add a first-class user-facing service category/group layer, default it to Basic Service, and continue querying the backend with the existing concrete service-name filter. This preserves legacy records and avoids rewriting/deleting stored logs.
- Producer mapping supports explicit categories without message-text guessing: Basic Service = Launcher/UI/Agent/Service/SelfHeal; Network & Capture = Capture/TUN/SystemProxy/NetworkMonitor/IPDetection; Core Runtime = Supervisor/sing-box/Mihomo/Xray/CoreUpdate; Subscription & Update = Subscription; unknown producer names fall into Other.
- The current Service method classifier already assigns TUN, Capture, Subscription, IPDetection, NetworkMonitor, or Service at emit time, so category derivation can be based solely on the persisted `service` field and remain deterministic for legacy records.
- There is no existing handler-level category test and the frontend has no composable test. Add store regressions for explicit persistence, category filtering, and legacy JSONL derivation; add a pure frontend catalog/default-query regression that can run under the existing Node test runner.
- UI/UX skill guidance favors a dense operational-dashboard treatment, semantic fieldsets/native controls, visible focus and selected state, async disable/loading feedback, and no decorative redesign. Reuse the existing theme tokens and filter layout; add a clearly labeled service-category fieldset before the raw service fieldset and expose the current default in text, not color alone.
- Implemented stable category IDs in the canonical log store: `basic_service`, `network_capture`, `core_runtime`, `subscription_update`, and `other`. Category is now serialized on each new JSONL record; classification depends only on the producer-supplied service name.
- Legacy JSONL entries without `category` are classified in memory during `Load`, so old files remain readable and queryable without destructive migration or rewrite. Unknown current/future service names fail into `other`.
- Query composition is conjunctive across selected severity levels, categories, concrete services, dates, and cursor. Clearing every category keeps the existing “no category restriction” behavior.
- The Vue log state starts with `selectedLogCategories=["basic_service"]`; metadata has a complete local fallback, while the paired backend exposes the canonical category ID catalog through `logs.categories`.
- Focused frontend tests now PASS 4/4, covering category defaults/labels, distinct semantic controls, and the API query contract.
- Full source gates PASS: `go test ./...`, `go vet ./...`, frontend 32/32 tests, and `npm run build` (Vue typecheck plus Vite, 68 modules).
- The webapp-testing skill's preferred Python Playwright binding is not installed on this host. Existing repository smoke assets already provide equivalent Wails bridge mocks and Node Playwright scripts, so browser verification can reuse that established local path without installing dependencies.
- Node Playwright and playwright-core are available in the bundled Codex runtime at `C:/Users/28484/.cache/codex-runtimes/codex-primary-runtime/dependencies/node/node_modules`; Microsoft Edge can be launched through the existing project smoke pattern. No dependency installation is required.
- Browser-visible acceptance PASS: Basic Service is checked on first open and the initial backend query contains only `basic_service`; INFO/WARN/ERROR are selected while DEBUG is not. Switching to Network & Capture plus concrete TUN returns only the TUN WARN record; excluding WARN produces the empty state.
- Browser theme/compact assertions PASS for day, night, and 760x900 with zero horizontal overflow and zero console/page errors. Screenshot files exist with hashes, but the managed image viewer ACL failure prevents claiming manual pixel inspection.
- Official 1.0.40 packaging PASS: directory 28 files/27 manifest entries, archive 28 files, both `issues_found=0`; repair check also reports zero issues.
- Navo-owned PE identity: `navo.exe` 1.0.40.0 SHA-256 `688FB67D88AC2832385806AF397291393B835A907533064CA54A46BC45F302F6`; `repair.exe` `8BF1F6AA338BBA51B5775FD62F0C9182454FAD9B5491F3FAFFDBFC330E0F38EC`; `app_ui/navo_app.exe` `8354630EF3C1744238F1978D9858BAF0DFCCFAA9DABB6D432D0F40BB25EFB5CA`.
- Final ZIP is 64,671,591 bytes with SHA-256 `66D57EC1334E375CBD5273A7C6CC9E87112CDE4FB246672B64E37D7F0FBA6517`. Packaging did not launch Navo; final Navo process count is zero.

## 2026-08-31 Phase 74 initial findings

- User-visible defect: V2 is enabled with one effective proxy, but the overview presents two boxes, labels the real-time curve as local proxy, and shows no detected exit; the three surfaces are not projected from one canonical committed connection snapshot.
- Existing architecture requires separate Selected/Candidate/Active semantics. Only `active_id` promoted after real data-plane verification can represent the effective outbound; a selected/candidate route may be shown as preparation detail but must not become a second active proxy card.
- Phase 36 history already defines traffic counters as capture-mode dependent, dual-link direct/proxy IP detection as partially available, and Dashboard as the compatibility boundary. Phase 72 later centralized Dashboard loading and stale-result suppression; this repair must preserve that single-flight loader.
- UI/UX skill guidance for this dense desktop operations screen favors one explicit state hierarchy, visible text and numeric evidence for charts, a legend/source label that is not color-only, and a meaningful pending/error state instead of a blank chart or omitted exit.
- The UI design-system search suggested a broad dark-only landing treatment and unsuitable marketing typography; those visual restyle recommendations are intentionally rejected. Navo must retain its existing day/night tokens, proportional Chinese UI type, dense diagnostic hierarchy, and focused behavior change.
- Live read-only evidence at triage time: no Navo process is running; v2rayN PID 17208 owns child sing-box PID 18036 from `D:\v2rayN-windows-64`, and current-user WinINet is enabled at `127.0.0.1:10808`. No process or network setting was changed.
- The exact split is architectural rather than a request-ID race: `environment.system_proxy` correctly marks the WinINet proxy as external/observe-only, while `capture.committed_mode` remains `off` because Navo does not own it.
- Overview permanently renders two peer IP cards (direct baseline and proxy exit). The hero still derives its route/mode from Navo `activeRoute` and committed capture, so an external active System Proxy reads as “尚未选择节点 / 未接管”.
- Traffic selection uses only `capture.committed_mode`: off selects physical-interface counters and “本地流量”; Navo System Proxy/TUN selects core metrics. External proxy activity therefore falls into the local branch even though WinINet is proxying applications.
- Service `ip.check` probes a proxy exit only when the Navo Supervisor is running and its detector is permanently configured for Navo's mixed port. When only v2rayN is active it returns proxy state `inactive`, so neither the dashboard background probe nor the manual IP check can produce the observed external exit.
- Chosen boundary: preserve Navo capture ownership and core metrics, but derive one read-only effective-connection projection. For an external System Proxy, show one canonical connection/exit card, probe its configured endpoint without mutating it, and label the curve as system-interface total traffic with an explicit “external proxy metrics unavailable” limitation instead of fabricating proxy-only counters.
- Red regressions were confirmed before production code: Agent failed to compile on the missing external endpoint/parser/check seam; all four frontend cases failed on the missing projection, duplicate overview cards, and raw capture-mode traffic selection.
- Agent now normalizes current Environment System Proxy endpoints (including WinINet per-protocol forms), rejects embedded credentials/unsupported schemes, runs direct and explicit-proxy detectors concurrently with bounded contexts, and caches evidence by effective connection key.
- Dashboard keeps its original Navo runtime exit compatibility but suppresses stale Navo exit evidence when the immutable Environment snapshot says the effective path is an external System Proxy. The response adds compatible connection-kind, provider, error, and checked-at fields.
- Manual `ip.check` is intercepted by Agent only for an observed external System Proxy while Navo capture is off; Navo/direct checks retain the existing Service path and child-request identity behavior.
- The Vue `effectiveConnection` computed is the sole primary presentation model. A committed Navo capture wins; an uncommitted selected/candidate route never wins over an observed external proxy; otherwise the presentation is direct.
- Overview now has one connection evidence card with current exit and a subordinate direct baseline row. The detailed read-only Environment aggregation moved to Network Detection, preserving access without duplicating the primary proxy surface.
- External traffic uses physical-interface counters under the label “系统总流量 / 物理网卡计数 · 外部代理只读”. Chart legend text becomes “系统总量上传/下载”; no Navo core counters or active-connection counts are fabricated.
- Focused result: Agent external/cached compatibility tests PASS; frontend projection/environment/state/traffic tests PASS 15/15; Vue typecheck PASS.
- The external overview action now says “检测外部代理出口” rather than claiming a Navo ChatGPT chain check. Its completion notice explicitly distinguishes dual-IP evidence from Navo-managed application readiness.
- The independent direct detector was reverified in `internal/ipdetect/echo.go`: it clones the default transport and sets `Proxy=nil`, while the external detector pins the observed WinINet endpoint with `http.ProxyURL`.
- Full source gates PASS: `go test ./...`, `go vet ./...`, frontend 36/36, Vue typecheck, Vite production build (69 modules), and `git diff --check`.
- The first complete affected Service run exposed a narrow pre-existing timing gate: the readiness test timed out at 10 seconds and Xray's concurrent version probe was empty. Both isolated tests passed, and a subsequent full Service package run passed in 41.221s; readiness alone consumed 9.16 seconds.
- Headless Edge external-proxy smoke PASS: exactly one overview connection card, zero legacy IP cards, zero overview Environment cards, verified exit/direct/endpoint evidence, system-total traffic labels and legends, Environment detail on diagnostics only, two themes, 760x900 with zero horizontal overflow, and zero console/page errors.
- Live read-only v2rayN evidence on the current host: WinINet remains `127.0.0.1:10808`; independent direct exit was `114.225.253.223`, explicit-proxy exit was `63.124.165.24`, Google returned 204, and GitHub returned 200. No Navo process was running and no proxy setting was changed.
- Official 1.0.41 packaging repeated all source gates and npm audit (0 vulnerabilities), then verified both directory and 28-file archive with `issues_found=0`; repair check also returned zero issues.
- The three Navo-owned executables embed file/product version 1.0.41.0. The 64,683,041-byte ZIP SHA-256 is `2F0FDB3E58E21C64248B7BFF4A08B695D3216DC18DC636C145C4E1E630123511`.
- Post-package state remains clean: Navo process count 0, Vite listener 4194 count 0, and the original v2rayN PID 17208 / sing-box PID 18036 plus WinINet endpoint remain unchanged.

## 2026-08-31 Phase 75 initial findings

- The reported `OUTBOUND_REQUIRED` response is the correct backend fail-closed result when Navo has no selected available outbound; the defect to investigate is why the current UI still offers or invokes the Navo capture action in the external-proxy/no-route state without an inline recovery path.
- The user also reports major layout problems after the Phase 74 functional unification, so this phase must inspect the whole shell/overview/responsive hierarchy rather than merely recolor the error row.
- Historical project guidance still applies: one primary connection hierarchy, committed-state truth, readable Chinese labels, stable desktop density, and no color-only status semantics. These constraints will be reverified against current source and browser rendering rather than assumed current.
- The required design-system search returned a marketing-style “feature-rich showcase / cinematic mobile” recommendation plus external Fira fonts. Those parts conflict with Navo's offline Windows diagnostic-console role and existing dual-theme tokens, so they are rejected rather than layered onto the product.
- Applicable design guidance: keep balanced modern variance, moderate dashboard density, one primary CTA, stable elevation, 4/8px spacing rhythm, visible legends/text, and paired light/dark contrast. Avoid decorative blur/glow, repeated CTAs, and layout-shifting effects.
- The focused UX search makes error recovery and responsive behavior mandatory: put a clear next action next to the route error, announce errors semantically, preserve visible focus, use at least 44px mobile targets with 8px gaps, reserve async space, and respect `prefers-reduced-motion`.
- Vue guidance supports keeping the existing Composition API/composable projection rather than adding a new global store solely for this refinement.
- Exact source attribution: the user's English reason string comes from `internal/agent/startup.go::requireConfiguredStartupRoute`, called by `startup.set` when enabling login startup. It is not the Service capture-admission message, whose wording is different.
- The normal capture composable already prevents a missing-route System Proxy/TUN request locally, but only writes a generic failure string. The Settings startup path lacks the same preflight and therefore reaches Agent, producing the logged `OUTBOUND_REQUIRED`.
- Overview still labels its primary action “启动 Navo 代理” when `captureRouteMissing` is true. Although `useCapture.setCapture` blocks the backend call, the CTA remains semantically misleading and has no direct route-selection action.
- Global feedback is a plain top-level paragraph with no recovery action. For route-required failures the correct surface is an inline, focusable callout with “前往连接管理”, while the persisted log should retain the backend code/reason/request ID.
- Current shell density is amplified by two-line descriptions on every sidebar item and several same-elevation cards in Overview. Layout refinement should reduce repeated explanatory text in navigation and create a clearer primary/secondary data hierarchy rather than just changing colors.
- Settings confirms the exact missing preflight: the startup checkbox calls `configureStartup(true, mode)` directly, while `useStartupSettings` only receives `execute` and has no Dashboard/route context. The Agent therefore owns the first rejection and emits the logged request.
- Connection Management already has a correct no-route pattern: capture choices are disabled, a capability note explains that external proxy ports are not borrowed, and the bottom Connect button is disabled. Phase 75 should reuse this truth instead of inventing another route state.
- The hard global rule `html, body, #app { min-width: 760px }` makes the existing 560px responsive block unreachable and guarantees horizontal overflow at 375px. This is a concrete layout defect, not a cosmetic preference.
- Current visual tokens mix two unrelated personalities: highly rounded blue/purple dark cards versus near-square orange day cards with step timing and heavy offset shadows. Shared geometry/elevation should be normalized while keeping theme-specific color.
- Typography is nominally 11/12/13px, but several page rules still hardcode 8–10px metadata and 9px filters/logs. This causes cramped Chinese text and poor compact-layout readability despite the large cards.
- `.startup-settings-card` forces four columns on desktop and only collapses at 860/560, which separates its status from the controls and contributes to the reported layout imbalance.
### Phase 75 layout follow-up (2026-08-31)

- The shell currently spends 82 px on a sticky page header while most feature pages repeat a second page introduction below it; this doubles hierarchy and consumes the first viewport before primary controls.
- The desktop sidebar is 246 px wide and every item permanently carries a second line. At 860 px it abruptly collapses to 72 px, so there is no intermediate compact state and the information density changes sharply.
- The startup card is a four-column grid (`240px + toggle + 160px + 190px`) and only collapses at 860/560 px; it is the concrete source of settings-page crowding.
- The global feedback is a centered floating paragraph without `role="alert"`, structured action, or a route-recovery affordance. It can report the problem but cannot lead the user out of it.
- Responsive CSS already contains a 560 px mode and reduced-motion override, but the root `min-width: 760px` prevents that mobile-width layout from ever being reached without horizontal overflow.
- All six new focused contracts fail against the current product: the shared route projection is absent, startup has no preflight, Overview has a raw core-state CTA, Settings/global feedback have no recovery control, themes override geometry/motion independently, navigation has no compact secondary-copy rule, and the shell retains the 760 px floor.
- Later-loaded `hierarchy.css` overrides the base page container to 1360 px and makes page introductions 86 px tall. Any layout repair must change the final cascade as well as the earlier shell/pages rules, or apparent source edits will be overridden.
- Implemented one pure `routeRecovery` projection shared by Overview, capture activation, and login startup: committed capture stops, a valid selected route starts, and the external-proxy/no-Navo-route state navigates to route selection.
- Login startup now checks the same route predicate before `apis.system.configureStartup`; the Agent `OUTBOUND_REQUIRED` gate remains unchanged as a race/process-boundary fallback.
- Shared visual geometry is now theme-independent, the shell header/sidebar are denser without losing accessible navigation descriptions, startup controls are grouped with inline route guidance, and the root can reflow from 320 px upward.
- The webapp-testing workflow requires managed server lifecycle, `networkidle` before DOM inspection, headless Chromium/Edge, captured console errors, descriptive selectors, screenshots, and explicit browser cleanup. Its server helper was invoked with `--help` before use as required.
- The existing proven Wails mock uses `SetCaptureMode` and `SetStartupSettings` as the exact bridge methods. The Phase 75 smoke deliberately makes both throw and counts calls, so a PASS proves missing-route interactions were stopped before IPC rather than merely accepted by a mock backend.
- Edge offender evidence identified the exact 375 px overflow: the global `input { width:100% }` rule also sized the startup checkbox to the full 267 px column, then its label text extended the row to 374 px. A shared semantic `.checkbox input[type=checkbox]` size rule fixes the control itself instead of clipping the card.
- Direct screenshot forwarding bypassed the managed viewer ACL for manual QA. The 375 px Overview has a clear single hero, stacked verification/route CTAs, and one canonical connection card; Settings stays within its rail/content split. The first images also exposed that system color preference must not be mistaken for explicit night coverage.
- Explicit night QA confirms the shared geometry no longer changes personality between themes and the 58 px icon rail leaves a readable 317 px content pane. The first steady-state candidate was photographed during the intentional 520 ms progress dismissal, so the visual harness now waits for `.activity-progress` to become hidden before capture.
- Final steady-state 1440 px night QA shows a balanced hero, 1.5/0.75 connection-risk hierarchy, subordinate monitor cards, and current-only navigation description. At 1024 px day, the remaining visual defects were evidence-table IP wrapping and a wrapped sidebar service label; the former now becomes one column below 1120 px and the latter truncates with a full-value title.
- The package failure is isolated to dependency transport: the official script intentionally uses `.cache/go-path/pkg/mod`, which lacked several modules; direct IPv6 to `proxy.golang.org` timed out. Current WinINet remains enabled at `127.0.0.1:10808`, owned by the existing v2rayN sing-box PID 18036, and the exact x/sys module returns HTTP 200 through that endpoint.
- The server lifecycle helper stopped its shell owners but left seven exact Navo Vite children from the Phase 75 runs; one still listened on 4195 and all could hold the Rolldown native binding. Each was identified by absolute workspace Vite path plus `--port 4195`, stopped selectively, and rechecked at zero; unrelated TimeBack Vite and v2rayN/sing-box processes were preserved.
- Official 1.0.42 packaging passed after the bounded transport/process fixes: full Go test/vet, clean npm install (0 vulnerabilities), frontend 42/42, typecheck/build, Wails production compile, PE metadata, directory verification, archive verification, and independent repair check all pass.
- The verified release contains 28 files with 27 manifest entries. `navo.exe`, `repair.exe`, and `app_ui/navo_app.exe` all embed 1.0.42.0; final ZIP is 64,684,118 bytes with SHA-256 `4981F15A7287F4632F720B2A0D3079AECA659FA8B9BB845E6B3AB18DCE76286B`.

## 2026-08-31 Phase 76 initial findings

- The requested priority model is contextual: explicit user intent should become the highest eligible work in its own domain, while safety rollback/shutdown and an already-entered commit section remain non-preemptible. A single global integer would incorrectly let unrelated diagnostics or route selection reorder network mutation ownership.
- Historical architecture already provides key boundaries to preserve: Agent serializes capture transitions, Service owns physical TUN/capture rollback, `runtimeState` separates Selected/Candidate/Active, and only a healthy commit promotes Active. The missing question is where queued background work and repeated preparation are still duplicated.
- Agent/Service/CoreHost are not interchangeable lifetimes. Supervisor/CoreHost owns a core process with an independent lifecycle context; Agent request contexts must not accidentally kill it. Phase 76 must measure which actual component restarts before introducing an `Agent TTL` label.
- The safest performance shape is an exact-key lease with health revalidation and fail-closed fallback. System Proxy may be able to reuse a healthy local core after releasing WinINet; TUN may only be able to reuse compilation/preparation because keeping a TUN-owning core alive can retain forbidden adapter/routes.
- `connection.Coordinator` is currently one global channel token. `Begin` only counts waiters and depends on channel wake order; `Request` has operation/origin/fault-domain but no intent, control domain, policy score, or supersession semantics.
- A capture request enters `PhaseApplying` before the Agent performs recovery, Service preparation, WinINet ownership mutation, data-plane verification, journal commit, and snapshot commit. Priority must therefore be decided before `PhaseApplying`; commit and rollback remain non-preemptible safety barriers.
- The only current capture fast path is an already-committed, healthy request for the same mode. After `off`, activation reaches `Service.capture.prepare` again and repeats preparation plus the external data-plane check.
- Agent process lifetime is already long-lived. The expensive lifetime boundary is below Agent: Service compile/start/stop and Supervisor/CoreHost state. A useful lease must attach to prepared/runtime evidence, not merely keep the Agent process alive.
- Observation IPC already bypasses the Coordinator. The new policy should rank only mutations within their affected domain while retaining the single global physical-network commit valve.
- Implementation checkpoint: replace implicit channel/FIFO admission with a deterministic policy queue ranked by `(intent, domain, operation)` and sequence; user capture choices outrank same-domain startup/background work, while safety rollback/shutdown remains highest.
- First implementation will not cancel an active applying/verifying/committing/rollback transaction. It will deterministically order queued work; stale-queue supersession requires an explicit caller-visible result and separate regression coverage.
- Service capture preparation must now be mapped precisely to split System Proxy running-core reuse from TUN preparation-only reuse. Active TUN adapter/routes must never survive a committed `off` state.
- `Service.prepareCaptureLocked` currently performs the full teardown first for every target: stop Supervisor/CoreHost, deactivate owned TUN networking, destroy the owned adapter, wait for absence, and clear TUN runtime evidence. Even `off` then recompiles a non-TUN configuration.
- Cold System Proxy activation then probes the selected outbound, compiles/applies System Proxy runtime, starts Supervisor, verifies ChatGPT application routing, and commits the healthy runtime. `Supervisor.Start` also executes network reconciliation before every core start.
- Cold TUN activation must retain its stronger sequence: preflight/direct-IP baseline, exact activation plan, pinned config compile, core start, adapter/routes/DNS activation, physical data-plane verification, and healthy commit. Those physical steps are not lease-skippable after `off`.
- Supervisor already gives the core a request-independent lifecycle context, so System Proxy process retention is technically possible. However, its monitor currently performs crash recovery while `StateRunning`; a warm-idle core needs an explicit no-restart state/policy before it can be safely retained after Agent commits `off`.
- A System Proxy warm lease must immediately release WinINet ownership at the Agent boundary while allowing only the loopback core to remain. Expiry, crash, policy/outbound/core mutation, Service shutdown, or health mismatch must synchronously invalidate/stop it before full activation proceeds.
- TUN warm optimization will be limited to immutable preparation evidence/config reuse when its exact key remains valid. It must still start the core, recreate/reverify the owned adapter/network transaction, and run the physical data-plane gate on every activation after `off`.
- Runtime compilation currently always emits and validates a timestamped candidate revision. If Supervisor is running, `applyRuntimeConfigLocked` performs a full `SwapConfig`; there is no content-hash no-op even when the effective configuration is unchanged.
- A successful runtime stores the exact `RevisionID`, `ConfigHash`, selected/active outbound, core ID, routing mode/lists, and config path. These fields can anchor a System Proxy warm identity; current core PID/state and loopback endpoint health must be added at reuse time.
- System Proxy compilation resolves and pins the selected endpoint DNS. Reuse therefore cannot be keyed only by selected ID: it must retain the compiled config hash/revision and expire quickly enough that endpoint/environment drift falls back to the cold resolver/compiler path.
- Runtime mutations already acquire `captureMu` and swap/verify the running core. This gives one invalidation seam: any mutation that changes the active revision/config hash necessarily makes the prior warm identity unequal, even if an explicit invalidation call is missed.
- Keeping the System Proxy core alive avoids both compile/validate and Supervisor reconcile/start. Agent still performs its post-Service data-plane probe before committing capture, so warm reuse can remain fast without claiming cached application readiness as fresh.
- TUN config cannot simply be retained as the current runtime after `off`: its generated config carries TUN enabled state, pinned route/interface evidence, and a session plan. Reuse will focus on bounded preflight evidence and parallel safe preparation, not starting an old TUN config without a new plan.
- The TUN preflight chain is strictly sequential today: platform/network preflight, independent direct-IP baseline, activation-plan construction (physical route plus endpoint resolution), manager creation, config compile, core start, network activation, and data-plane verification.
- The direct-IP baseline and activation-plan construction are read-only before any owned network mutation and can run concurrently after platform preflight. This is a safe latency reduction that preserves a fresh session, route, endpoint pin, and baseline on every TUN activation.
- TUN preparation TTL can cache only stable prerequisite evidence such as a successful selected-outbound TCP check or core capability; platform/route/DNS/direct-IP evidence must remain fresh. The current TUN flow does not call `verifySelectedOutboundReachable`, so adding a redundant cached TCP probe would not reduce its present critical path.
- Agent rollback always disables WinINet and calls Service `capture.prepare(off)` on activation failure. A warm-off path must distinguish ordinary user disable from failure rollback; failures must force cold cleanup rather than preserve the just-failed core.
- `prepareServiceCapture` currently sends only `mode`. Add a bounded internal `retain_warm` flag only for an ordinary transition from committed System Proxy to `off`; rollback/recovery/off from TUN must omit it and therefore force full cleanup.
- Warm Service payload may expose `warm_reused`/lease timing for latency evidence, but the Agent must still derive committed state from its own successful journal/snapshot transaction and never from that optimization hint.
- The production combined launcher (`cmd/navo`) injects `CaptureRouteProbeFn`; warm System Proxy activation therefore retains a fresh post-Service acceptance gate in the shipped desktop path. Standalone Agent configurations without that callback must not claim the cached Service verification timestamp as fresh.
- Priority policy fixed for regressions: safety/shutdown is highest; explicit UI/tray interaction next; recovery/self-heal next; startup next; scheduler/background last. Domain is derived from operation/fault domain and controls same-domain supersession; equal-intent work stays FIFO to preserve user action order.
- A newer queued request may supersede only lower-intent queued work in the same domain. It cannot supersede an equal-priority user request, unrelated-domain work, or the active transaction. Superseded callers receive typed `ErrSuperseded` evidence rather than a timeout.
- System Proxy warm TTL will be a Service configuration with a conservative default and deterministic clock/timer seams. The exact duration is policy, while the validity condition remains runtime identity plus running PID/core health; negative configuration can disable it for tests/deployments.
- Red contracts must cover policy mapping, priority dequeue, same-domain lower-priority supersession, active non-preemption, lease expiry/key mismatch/crash suppression, ordinary-off `retain_warm`, rollback cold cleanup, and concurrent TUN read-only preparation.
- Existing Supervisor tests provide a controllable CoreHost with start-call/PID/state evidence, so warm-idle crash behavior can be verified without launching a real proxy process.
- Existing Agent tests already separate explicit user transitions from direct `captureFailure` rollback and expose exact Service messages. The new regression can prove only the former emits `retain_warm=true`, while rollback `capture.prepare(off)` does not.
- Production uses Go 1.26.4, so the Coordinator can use typed waiter structs and `sync.Mutex.TryLock` without compatibility shims; deterministic tests should still avoid scheduler-order assumptions and long sleeps.
- Added the Phase 76 red contracts across Coordinator, Supervisor, Agent, and Service. The first focused run did not reach compilation because the isolated module cache attempted direct `proxy.golang.org` downloads and timed out; this is an environment transport failure, not a product-test result.
- Process-local proxy retry reached the intended red state: Coordinator lacks Intent/Domain/policy queue, Supervisor lacks warm-idle controls, Service lacks lease/concurrent-preparation types, and Agent ordinary disable omits `retain_warm`; rollback test already passes by omission.
- Production `cmd/navo` final probes are physical current-user probes, not a second Service `runtime.verify`: System Proxy calls `ProbeDefaultProxy` (or direct-routing variant), while TUN calls no-explicit-proxy `ProbeDirect` (or direct-routing variant). Warm reuse therefore still performs independent host-path acceptance.
- `Supervisor.Status` refreshes PID/ConfigHash from CoreHost only while running. Lease reuse can require Supervisor running, same PID, runtime revision/config/path identity, and optionally matching host config hash when available.
- Implemented Coordinator admission as an explicit waiter set rather than channel receiver order. Selection is highest intent rank then oldest sequence; cancellation removes a pending waiter or immediately releases a just-granted transaction so ownership cannot leak.
- Same-domain supersession is deliberately asymmetric: a new higher-rank waiter fails older lower-rank waiters with typed `ErrSuperseded`; equal-rank user actions and unrelated domains remain queued FIFO, and the active transaction is untouched.
- Every transaction snapshot now carries resolved Intent, Domain, and Priority evidence, while request IDs, phases, fault domains, last result, and one global mutation owner remain compatible.
- Coordinator focused suite is green, including interactive-before-startup admission, same-domain lower-priority supersession, active non-preemption, cancellation, TryBegin busy evidence, and failure snapshots.
- Supervisor now has an explicit warm-idle flag separate from transient restart suppression. Start/Stop clear it; a warm-idle process exit transitions to stopped without restart or recovery-budget consumption, so committed OFF cannot resurrect the retained proxy core.
- Full Supervisor package tests PASS after the lifecycle change, including existing start/stop/restart/config-swap/crash-monitor behavior and the new warm-idle crash gate.
- Agent now distinguishes explicit user/tray disable from internal OFF transitions. Only an explicit transition whose committed source is System Proxy sends `retain_warm=true`; capture failure, recovery, node/source/core work, and policy rollback retain the existing cold `capture.prepare(off)` path.
- The optimization hint is internal Agent-to-Service metadata and does not alter the UI request ID or Service child-ID allocation, preserving at-most-once replay and UI correlation.
- Agent full package tests PASS after option threading, including the new explicit warm-disable and failure cold-cleanup regressions plus all existing capture/recovery/coordinator cases.
- Added a pure System Proxy lease model with exact equality across core, selected outbound, active revision, config hash/path, optional host config hash, and PID. Reuse also requires Supervisor running and strict pre-expiry time; equality cannot be bypassed by a matching mode alone.
- Service policy defaults the loopback-core retention window to 90 seconds; a negative configured duration disables retention. The clock and duration live at the Service boundary so expiry behavior can be deterministic in tests and independent of UI request deadlines.
- Service lease runtime now performs a fresh CoreHost health check before both retain and resume, keeps a generation-bound timer, retries expiry cleanup only when the capture mutex is busy, and stops the retained core under the same Service capture mutex after expiry.
- `capture.prepare(system_proxy)` attempts exact warm resume before any teardown. A hit cancels expiry and returns the existing PID/config evidence; a miss invalidates the lease and enters the unchanged full stop/cleanup/compile/reconcile/start/verify/commit path.
- `capture.prepare(off, retain_warm=true)` still deactivates any owned TUN network, destroys/waits for the owned adapter, and clears TUN runtime evidence before marking the loopback core warm-idle. OFF without the flag invalidates the lease and keeps full cold cleanup.
- A successful cold System Proxy activation records the lease only after Service application-routing verification and healthy runtime commit. Lease hints are returned as additive payload fields and do not alter capture commit semantics.
- TUN now runs its fresh direct-IP baseline and fresh activation-plan build concurrently after platform preflight. Both finish before compile or physical mutation; errors are joined, cancellation propagates, and every activation still gets a new session/route/endpoint plan and full data-plane verification.
- Runtime config application detects a warm-idle core, invalidates its lease, and stops it before compiling a mutation. Service shutdown also cancels the lease timer and clears warm-idle before normal core/TUN teardown, preventing timer or old PID ownership from crossing lifecycle boundaries.
- First Service compile found a narrow static mismatch: System Proxy stores `RuntimeRoutingVerification`, while the lease test/model was initially typed as the TUN-only `VerifyResult`. No runtime test executed; correct the lease to the System Proxy evidence type rather than weakening it to `any`.
- Warm evidence storage and tests now use the exact `RuntimeRoutingVerification` type, preserving verified sites, direct/proxy exit identity, and verification time as structured evidence.
- The next compiler pass found the two retain/resume method signatures still declared the old return type even though their storage/bodies were corrected. Updated both signatures to the same strong System Proxy type; no runtime path executed before this correction.
- Service full package tests PASS in 40.122s after the strong-type fix. This covers the new exact lease and concurrent TUN preparation contracts plus existing runtime verification, Candidate/Active commit, capture rollback, IPC replay, core/source mutation, and service lifecycle regressions.
- Added a real Supervisor/CoreHost-backed Service lease fixture (no external process/network mutation) to verify the integrated lifecycle: retain then resume keeps the same PID without stop, config-hash drift rejects reuse, and a short lease timer stops the warm core exactly once.
- Service full package rerun PASS in 28.750s with all three lifecycle integrations. The timer test confirms one clean stop and no retained warm-idle state after expiry.
- Complete diff check reports no whitespace errors; only the repository's existing CRLF normalization notices appear. Prior Phase 73-75 UI/release changes remain preserved and are not rewritten by Phase 76.
- Read-through found one TUN concurrency refinement: the first implementation starts both reads concurrently but receives the direct-IP result first, so a fast plan failure cannot cancel a slow direct reader immediately. Replace fixed receive order with a two-channel select loop before final all-repository gates.
- Coordinator snapshots are consumed by Agent status/environment payload builders. Inspect and extend those explicit maps so resolved intent/domain/priority become user-visible evidence instead of remaining internal struct fields only.
- Exact Agent capture payload currently projects transaction busy/id/operation/origin/phase/fault-domain/timing but omits the new policy fields. Add intent/domain/priority there; Environment has a typed `TransitionSnapshot`, so its schema must be inspected and extended compatibly before projection.
- Both Go Environment and frontend Dashboard/Environment transaction DTOs are additive object shapes. Optional `intent`, `domain`, and numeric `priority` fields preserve compatibility with older Agent responses while making the selected policy observable in logs/status/UI tooling.
- TUN result collection now selects whichever read completes first and cancels the sibling immediately on failure; success still waits for both fresh inputs before any mutation.
- Capture status and Environment now project resolved intent/domain/priority; frontend types accept the fields optionally for mixed-version compatibility. This makes every active queued policy decision inspectable without changing UI actions.
- Full repository `go test ./...` PASS after the final refinements, including Agent, Service (31.341s), Coordinator, Supervisor, network/TUN, network environment, IPC, startup, self-heal, core adapters, and Wails application packages.
- Full `go vet ./...` PASS with no findings.
- Full frontend suite PASS 42/42 after the additive transaction DTO fields; all Phase 73-75 effective-connection, route-recovery, layout, logging, dashboard single-flight, capture polling, and traffic contracts remain green.
- Vue TypeScript check and Vite production build PASS with 70 transformed modules.
- Coordinator and Supervisor packages each PASS 20 consecutive runs; priority admission, cancellation/supersession, transaction ownership, warm-idle crash suppression, and existing lifecycle state tests show no timing flake in this repeated gate.
- Warm lease lifecycle and concurrent TUN preparation focused cases PASS 20 consecutive runs (1.502s total), including timed stop and exact drift rejection.
- Admission audit found existing Agent callers map every `Begin` error to busy/general capture failure. Add one shared typed mapper so `ErrSuperseded` becomes `REQUEST_SUPERSEDED` while retaining each caller's existing busy code for real contention/cancellation.
- Added the shared admission mapper across capture/core/node/source/policy/core-update/tray recovery entry points and direct capture transition handling. `ErrSuperseded` is now explicit `REQUEST_SUPERSEDED`; existing `CAPTURE_BUSY`/`CONNECTION_BUSY` behavior remains unchanged for actual contention.
- All four affected packages PASS after formatting and the typed error refinement: Agent 2.763s; Coordinator, Service, and Supervisor cached green.
- Final post-refinement full `go test ./...` PASS and final `go vet ./...` PASS. The frontend 42/42 and production build remain valid because no frontend source changed after that gate.
- Final pre-package frontend rerun PASS 42/42; Vue typecheck and Vite production build PASS again with 70 modules.
- Pre-package hygiene: exact Navo executable process count zero, Navo workspace Vite count zero, port 4195 listener count zero, and `git diff --check` has no errors beyond CRLF notices.
- External environment remains untouched and current: WinINet is enabled at `127.0.0.1:10808`; listener PID 8936 is the preserved v2rayN sing-box child and v2rayN PID is 14404. No external process/configuration was stopped or read.
- Official 1.0.43 package script PASS: it independently repeated full Go tests, npm clean install/audit (0 vulnerabilities), frontend 42/42, typecheck/build (70 modules), Wails production build, directory verification, and 28-file archive verification with `issues_found=0`.
- Official outputs: `release/Navo-1.0.43-portable-amd64` and `release/Navo-1.0.43-portable-amd64.zip`; reported ZIP SHA-256 is `9F9FA7B1CE73AD2CEB27440AAE613A21D8F3B663310595A76EC15971F4AC4F20`.
- Independent `verify-package.ps1` rerun PASS: version 1.0.43, 28 directory files, 27 manifest entries, 28 archive files, and `issues_found=0`.
- Packaged `repair.exe check` PASS with `issues_found=0` and `fixable=false`.
- Independent ZIP hash matches the official value and size is 66,300,689 bytes. The first PE metadata display was table-truncated, so rerun with explicit key/value output before recording executable versions/hashes.
- Explicit PE verification is complete: `navo.exe` 1.0.43.0 / `F88C12D59A34F537BABF35E1EBBFA99D3A13CD31E78ADA0DF34C0F1D20711FF7`; `repair.exe` 1.0.43.0 / `4D49313AD716F2109E4B507CE8E3924C8F7B1A04A6F464FFCB5E1BD29435FF65`; `app_ui/navo_app.exe` 1.0.43.0 / `77470BF8887622F81A963015015F0E1BB040A282C20CBEE24BD47FDE35B50A4E`.
- Final residue check is clean for Navo: process count 0, Navo Vite count 0, and no listener on 4195 or 12080. The external WinINet endpoint is unchanged at `127.0.0.1:10808`, with preserved sing-box PID 8936.
- Phase 76 deliberately does not claim measured physical latency reduction: the same-PID/no-stop-start System Proxy path and TUN parallel-preparation path are structurally and deterministically verified, but live activation would require a configured Navo outbound and would mutate the user's current network state.
